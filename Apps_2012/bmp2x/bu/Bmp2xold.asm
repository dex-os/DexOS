...
msg2:	db " Error!, file not found. ",10,13
	db " Press any key to exit. ",10,13,0
msg3:	db " Error!, No comandline, BMP or GIF file found. ",10,13
	db " Press any key to exit. ",10,13,0
msg4:	db " Error!, Not a valid BMP or GIF file. ",10,13
	db " Press any key to exit. ",10,13,0
msg8:	db " File Converted successfully, thankyou for using Bmp2X!.",10,13
	db " Press any key to exit. ",10,13,0
 ;----------------------------------------------------;
 ; Start of program.                                  ;
 ;----------------------------------------------------;
...
ConvetTo565Loop:
	...
	lodsb
	mov   bl,al
	mov   dword[rgb_color],ebx
	call  MakeColor16
	mov   ax,word[Conveted]
	stosw
	loop  ConvetTo565Loop
	popad
	mov   esi,ScreenBuffer1
;-------------------
 ;----------------------------------------------------;
...
;===========================================================
; transform a 24 bit RGB color
; into 16bit 5:6:5 video mode color
;
; - should be redefined for multiple color resolutions
; - pathetically unoptimised
; - do not use for every pixel opperations!!!
;===========================================================
MakeColor16:
	pushad
	xor	edx,edx
	...
	; RRRRR_GGGGG_BBBBB
	; doubled all over eax
      ;  mov     eax,edx
     ;   shl     edx,16
     ;   or      eax,edx
	mov	word[Conveted],dx
	popad
...
MsgLogo1: 
db  0xd2,0x09,';***********************************;',13
	   db ';  ____                 ____ __  __ ;',13
	   db '; | __ ) _ __ ___  _ __|___ \\ \/ / ;',13
	   db "; |  _ \| '_ ` _ \| '_ \ __) |\  /  ;",13
	   db '; | |_) | | | | | | |_) / __/ /  \  ;',13
	   db '; |____/|_| |_| |_| .__/_____/_/\_\ ;',13
	   db ';                 |_|               ;',13
	   db ';***********************************;',10,10,13,13
db 0xd2,0x07, ' Gif/Bmp to Hex converter, for DexOS, version ',VERSION_STRING,13
	   db ' Press any key to load & convert image . ',13,13,0
MsgLogo2:  
	   db ' Loading and converting image, please wait..... ',10,10,13,13,0
